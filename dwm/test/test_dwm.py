...
def test_history_fieldSpecificLookup_uncaught():
    ...
    dataOut = dwmAll(data = test_records.history_fieldSpecificLookup_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_lookupAll_fieldSpecificLookup')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_fieldSpecificLookup_notChecked():

    dataOut = dwmAll(data = test_records.history_fieldSpecificLookup_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_lookupAll_fieldSpecificLookup')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field2' not in hist.keys()
...
def test_history_normLookup_caught():
    ...
    # do stuff for testing
    dataOut = dwmAll(data = test_records.history_normLookup_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_lookupAll_normLookup')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert hist['field1']['normLookup']['from'] == 'badvalue'

def test_history_normLookup_uncaught():

    dataOut = dwmAll(data = test_records.history_normLookup_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_lookupAll_normLookup')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_normLookup_notChecked():

    dataOut = dwmAll(data = test_records.history_normLookup_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_lookupAll_normLookup')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field2' not in hist.keys()
...
def test_history_genericRegex_caught():
    ...
    dataOut = dwmAll(data = test_records.history_genericRegex_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_genericRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert hist['field1']['genericRegex']['from'] == 'badvalue'

def test_history_genericRegex_uncaught():

    dataOut = dwmAll(data = test_records.history_genericRegex_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_genericRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_genericRegex_notChecked():

    dataOut = dwmAll(data = test_records.history_genericRegex_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_genericRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field2' not in hist.keys()
...
def test_history_fieldSpecificRegex_caught():
    ...
    dataOut = dwmAll(data = test_records.history_fieldSpecificRegex_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_fieldSpecificRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
...
def test_history_fieldSpecificRegex_notChecked():
    ...
    dataOut = dwmAll(data = test_records.history_fieldSpecificRegex_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_fieldSpecificRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field2' not in hist.keys()
...
def test_history_normRegex_caught():
    ...
    dataOut = dwmAll(data = test_records.history_normRegex_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_normRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
...
def test_history_normRegex_uncaught():
    ...
    # do stuff for testing
    dataOut = dwmAll(data = test_records.history_normRegex_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_normRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_normRegex_notChecked():

    dataOut = dwmAll(data = test_records.history_normRegex_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_regexAll_normRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field2' not in hist.keys()
...
def test_history_deriveValue_caught():
    ...
    dataOut = dwmAll(data = test_records.history_deriveValue_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveValue')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert hist['field1']['deriveValue']['from'] == ''

def test_history_deriveValue_uncaught():

    dataOut = dwmAll(data = test_records.history_deriveValue_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveValue')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_deriveValue_notChecked():

    dataOut = dwmAll(data = test_records.history_deriveValue_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveValue')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field3' not in hist.keys()

def test_history_deriveValue_overwriteFalse():

    dataOut = dwmAll(data = test_records.history_deriveValue_overwriteFalse, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveValue_overwriteFalse')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()
...
def test_history_copyValue():
    ...
    dataOut = dwmAll(data = test_records.history_copyValue, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_copyValue')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
...
def test_history_deriveRegex_caught():
    ...
    dataOut = dwmAll(data = test_records.history_deriveRegex_caught, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert hist['field1']['deriveRegex']['from'] == ''

def test_history_deriveRegex_uncaught():

    dataOut = dwmAll(data = test_records.history_deriveRegex_uncaught, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()

def test_history_deriveRegex_notChecked():

    dataOut = dwmAll(data = test_records.history_deriveRegex_notChecked, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveRegex')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field3' not in hist.keys()

def test_history_deriveRegex_overwriteFalse():

    dataOut = dwmAll(data = test_records.history_deriveRegex_overwriteFalse, mongoDb = db, mongoConfig=mongoConfig, configName='test_deriveAll_deriveRegex_overwriteFalse')
    hist = db.contactHistory.find_one({"_id": dataOut[0]['historyId']})
    assert 'field1' not in hist.keys()
